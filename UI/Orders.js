const ORDERS_API_URL = window.location.protocol === 'file:' ? 'http://localhost:4000' : '';
...
function getFilteredRows() {
    ...
    if (ordersState.status !== 'all') {
        rows = rows.filter(x => String(x.status || '').toLowerCase() === ordersState.status);
    }
    ...
    if (ordersState.search) {
        ...
        rows = rows.filter(x =>
            ...
            String(x.tableLabel || '').toLowerCase().includes(keyword) ||
            String(x.itemSummary || '').toLowerCase().includes(keyword)
        );
    }
...
function renderKpis(summary) {
    ...
    if (completedBtn) completedBtn.textContent = `Hoàn thành (${summary.completedOrders || 0})`;
}
...
function renderRows() {
    ...
    tbody.innerHTML = rows.map((row, index) => {
        ...
        const paymentLabel = isCompleted
            ? (String(row.paymentMethod || '').toLowerCase().includes('cash') ? 'Tiền mặt' : 'Chuyển khoản')
            : 'Đang mở';
        const tableIcon = String(row.tableLabel || '').toLowerCase().includes('bàn') ? 'fa-chair' : 'fa-bag-shopping';
    ...
    const info = document.getElementById('ordersPaginationInfo');
    if (info) info.textContent = `Đang hiển thị 1 đến ${rows.length} trong số ${rows.length} kết quả`;
}

function wireFilters() {
    document.querySelectorAll('.dash-order-filter-tabs .dash-filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('.dash-order-filter-tabs .dash-filter-btn').forEach(x => x.classList.remove('active'));
...
window.onEditOrder = function onEditOrder(orderId) {
    window.showToast?.(`Đơn #${orderId}: chức năng Sửa sẽ bổ sung ở bước tiếp theo.`, 'info');
};

window.onDeleteOrder = function onDeleteOrder(orderId) {
    window.showToast?.(`Đơn #${orderId}: chưa hỗ trợ xóa mềm từ màn hình này.`, 'warning');
};
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        window.showErrorToast?.(error.message || 'Không thể tải danh sách đơn hàng');
    }
});
