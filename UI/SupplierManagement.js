...
function renderRows(suppliers, importStats) {
  ...
  tbody.innerHTML = suppliers
    .map((supplier) => {
      ...
      return `
            <tr>
                <td class="td-checkbox"><input type="checkbox" class="row-checkbox" value="${supplierId}" ${selectedSupplierIds.includes(supplierId) ? "checked" : ""} /></td>
...
function updateBulkDeleteButton() {
  ...
  if (btn) {
    btn.style.display =
      selectedSupplierIds.length > 0 ? "inline-block" : "none";
    btn.innerHTML = `<i class="fa-solid fa-trash"></i> Xóa Đã Chọn (${selectedSupplierIds.length})`;
  }
}
...
async function editSupplierInternal(supplierId) {
  const current = supplierState.suppliers.find(
    (s) => Number(s.supplierId) === Number(supplierId),
  );
  if (!current) return;
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
      ...
      addButton.addEventListener("click", async () => {
        try {
          ...
          window.showErrorToast?.(
            error.message || "Không thể thêm nhà cung cấp",
          );
        }
      });
    }
    ...
    if (tableBody) {
      tableBody.addEventListener("change", (e) => {
        if (e.target.classList.contains("row-checkbox")) {
          ...
          if (e.target.checked) {
            if (!selectedSupplierIds.includes(id)) selectedSupplierIds.push(id);
          } else {
            selectedSupplierIds = selectedSupplierIds.filter((x) => x !== id);
          }
    ...
    if (btnBulkDelete) {
      btnBulkDelete.addEventListener("click", async () => {
        ...
        const confirmed = await window.showConfirmModal(
          `Bạn có chắc muốn xóa ${selectedSupplierIds.length} nhà cung cấp đã chọn?`,
        );
        if (!confirmed) return;

        try {
          for (const id of selectedSupplierIds) {
            await supplierApiRequest(`/suppliers/${id}`, "DELETE");
          }
          window.showSuccessToast?.(
            `Xóa ${selectedSupplierIds.length} nhà cung cấp thành công`,
          );
        } catch (err) {
    ...
    window.showErrorToast?.(
      error.message || "Không thể tải trang nhà cung cấp",
    );
  }
});
...
window.deleteSupplier = async function (supplierId) {
  try {
    const supplier = supplierState.suppliers.find(
      (s) => Number(s.supplierId) === Number(supplierId),
    );
    const supplierName =
      supplier?.name || `#NCC-${String(supplierId).padStart(3, "0")}`;
