...
function renderRevenueTrend(revenueTrend, trendLabels) {
  const bars = document.querySelectorAll(
    ".dash-chart-container .dash-chart-bar",
  );
  const labels =
    Array.isArray(trendLabels) && trendLabels.length
      ? trendLabels
  ...
  const labelNodes = document.querySelectorAll(
    ".dash-chart-container .dash-chart-label",
  );
...
function renderRecentOrders(recentOrders) {
  ...
  listNode.innerHTML = recentOrders
    .slice(0, 5)
    .map(
      (order, index) => `
        <div class="dash-order-item">
...
function renderLowStock(lowStockIngredients) {
  const tableBody = document.querySelector(
    ".dash-bottom-grid .dash-table-card .dash-table tbody",
  );
  if (!tableBody) return;
  ...
  tableBody.innerHTML = lowStockIngredients
    .slice(0, 5)
...
function renderShiftTeam(openShiftStaff) {
  ...
  const actionHtml = `
        <div class="dash-team-action-container">
          <button class="dash-team-btn" onclick="window.location.href='/app/staff'">Quản Lý Lịch Trực</button>
        </div>`;
  ...
  const members = openShiftStaff
    .slice(0, 4)
    .map((shift, index) => {
      const name = shift.employeeName || `NV #${shift.employeeId}`;
      const parts = name.split(" ").filter(Boolean);
      const initials =
        (parts[0]?.[0] || "N") + (parts[parts.length - 1]?.[0] || "V");
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    window.showErrorToast?.(
      error.message || "Không thể tải dữ liệu trang tổng quan",
    );
  }
});
